...
- 消費は **基礎代謝 + 実測した活動** で出す（活動レベル係数は使わない）。
...
### 4.2 維持カロリー（measured maintenance）
「活動を記録した日」の実測平均。立ち上がりは係数で仮置き。
```
対象 = days のうち activities.length > 0 の日（直近21日まで）
各日の burn = BMR(その日時点の最新体重) + Σactivities.kcal
対象が5日以上 → 平均を維持カロリー（source: "measured"）
5日未満       → BMR * 1.5（COLDSTART_FACTOR、source: "estimate"）
```
...
- **体重**: 1入力（必須）。記録済みは✓表示。
- **活動**: 「消費 = 基礎代謝 + 活動」の内訳表示（ホームと数字一致）＋ 活動追加（ウォーキングはMET概算電卓あり: `kcal ≒ MET × 体重 × 時間`）。
...
1. **消費 = 基礎代謝 + 実測活動**。活動レベル係数（1.4/1.55等）は廃止済み。表からも計算からも出さない（COLDSTART_FACTOR だけ維持カロリーの初期推定に裏で使う）。
