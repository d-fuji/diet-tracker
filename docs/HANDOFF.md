...
1. **維持カロリー(TDEE) = 基礎代謝×NEAT係数 + 実測活動 + TEF**（2026-06 方針転換）。当初は「実測主義」で活動レベル係数を廃止していたが、NEAT（無意識の日常活動）と TEF（食事誘発性熱産生）が抜け落ち維持カロリーを過小評価していたため、NEAT係数（日常活動量で可変・既定1.2）と TEF（摂取×0.10）を導入した。なお**ホーム/記録画面の「今日の消費」表示は従来どおり基礎代謝＋実測活動のまま**（その日に実際に消費・記録した実数を見せる役割。NEAT/TEF は推定値なので“計画用の維持カロリー”側にのみ反映する）。
